...
public class ChatUtil {
	public static String getName(final ItemStack stack) {
		if (stack.hasTag() && stack.getTag().hasKeyOfType("display", 10)) {
			final NBTTagCompound nbttagcompound = stack.getTag().getCompound("display");
		...
		if (stack.getType() == Material.POTION && stack.getData().getData() == 0) {
			...
			if (potion != null) {
				...
				if (type != null && type != PotionType.WATER) {
					final String effectName = (potion.isSplash() ? "Splash " : "")
	...
	public static Trans fromItemStack(final org.bukkit.inventory.ItemStack stack) {
		...
		nms.save(tag);
		return new Trans(getName(nms), new Object[0]).setColor(ChatColor.getByChar(nms.u().e.e()))
