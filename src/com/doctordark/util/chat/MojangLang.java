...
import com.doctordark.util.MoreObjects;
 ...
 * translatable strings into the language of Minecraft, as well as retrieving
 * the translatable representations of Items and Enchantments to be sent to the
 * client using raw chat message.
 */
public abstract class MojangLang {
	...
	/**
	 * Reads the translations of the server for a specific Minecraft version and
	 * locale.
	 *
	 * @param minecraftVersion the Minecraft version, e.g. 1.7.10
	 * @param locale           the locale to translate from
	 * @throws IOException              if the source could not be read
	 * @throws IllegalArgumentException if the Locale for the given Minecraft
	 *                                  version is already indexed
	 */
	public void index(String minecraftVersion, @Nullable Locale locale) throws IllegalArgumentException, IOException {
		...
		checkArgument(!translations.contains(locale, minecraftVersion), "Already indexed Minecraft version '"
				+ minecraftVersion + "' for locale '" + locale.toLanguageTag() + "'.");
	}
	 ...
	 * placeholders, the args will be substituted using String.format() eg.
	 * {@link MojangLang}.translate("record.nowPlaying", "My favourite Song") ->
	...
	public String translate(Locale locale, String key, Object... args) {
		checkArgument(translations.containsRow(locale), "Translations for locale " + locale.toLanguageTag()
				+ " not initialised, use #index(String, Locale) to index");
	 ...
	 * eg. Lang.translatableFromStack(new ItemStack(Material.INK_SACK, 1, 4)) =
	 * "item.dye.blue"
	 *
	...
	/**
	 * get the name of the provided item translated to Mojang's language eg.
	 * {@link MojangLang}.fromStack(new ItemStack(Material.INK_SACK, 1, 4)) = "Lapis
	 * Lazuli"
	 *
	 * @param stack a Bukkit ItemStack
	 ...
	 * eg. {@link MojangLang}.translatableFromEnchantment(Enchantment.DIG_SPEED) =
	 * "enchantment.digging"
	 *
	...
	public static String translatableFromEnchantment(final Enchantment ench) {
		final net.minecraft.server.v1_8_R3.Enchantment nms = net.minecraft.server.v1_8_R3.Enchantment
				.getByName(ench.getName());
		return (nms == null) ? ench.getName() : nms.a();
	}

	/**
	 * Gets the name of the provided enchantment translated to the Mojang's language
	 * eg. {@link MojangLang}.fromEnchantment({@link Enchantment#DIG_SPEED} =
	 * "enchantment.digging"
	 *
	 * @param enchantment a Bukkit Enchantment
	...
	/**
	 * Gets the translatable string representing the name of the provided potion
	 * effect. eg.
	 * {@link MojangLang}.translatableFromEnchantment(PotionEffectType.FAST_DIGGING)
	 * = "potion.digSpeed"
	 *
	...
	/**
	 * Gets the name of the provided Potion Effect translated to the Mojang's
	 * language eg.
	 * {@link MojangLang}.fromPotionEffectType({@link PotionEffectType#FAST_DIGGING})
	 * = "potion.digSpeed"
	 *
	 * @param effectType a PotionEffectType
