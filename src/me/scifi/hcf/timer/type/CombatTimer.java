...
public class CombatTimer extends PlayerTimer implements Listener {
	...
	public CombatTimer(HCF plugin) {
		super(plugin.getMessagesYML().getString("SCOREBOARD.COMBAT.NAME"),
				TimeUnit.SECONDS.toMillis(HCF.getPlugin().getMessagesYML().getLong("SCOREBOARD.COMBAT.LENGTH")));
		this.plugin = plugin;
	}
	...
	public void onFactionJoin(PlayerJoinFactionEvent event) {
		...
		if (optional.isPresent()) {
			...
			if (remaining > 0L) {
				...
				player.sendMessage(Utils.chat(ChatColor.RED + "You cannot join factions whilst your " + getDisplayName()
	...
	public void onFactionLeave(PlayerLeaveFactionEvent event) {
		...
		if (optional.isPresent()) {
			...
			if (remaining > 0L) {
				...
				if (sender == player) {
					sender.sendMessage(Utils.chat(ChatColor.RED + "Cannot kick " + player.getName() + " as their "
		...
		if (!event.getFromFaction().isSafezone() && event.getToFaction().isSafezone() && getRemaining(player) > 0L) {
			...
			player.sendMessage(Utils.chat(
					ChatColor.RED + "You cannot enter " + event.getToFaction().getDisplayName(player) + ChatColor.RED
							+ " whilst your " + getDisplayName() + ChatColor.RED + " timer is active."));
		}
	}
	...
	public void onEntityDamageByEntity(EntityDamageByEntityEvent event) {
		...
		if (attacker != null && (entity = event.getEntity()) instanceof Player
				&& !plugin.getStaffModeManager().getStaffMode().contains(attacker.getUniqueId())) {
	...
	public void onTimerStart(TimerStartEvent event) {
		if (event.getTimer() == this) {
			...
			if (optional.isPresent()) {
				...
				Player player = optional.get();
				player.sendMessage(Utils.chat(
						plugin.getMessagesYML().getString("COMBAT-TAG-MESSAGE").replace("%displayname%", name).replace(
								"%duration%", DurationFormatter.getRemaining(event.getDuration(), true, true))));
			}
		}
	}
