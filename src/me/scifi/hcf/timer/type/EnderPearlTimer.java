...
public class EnderPearlTimer extends PlayerTimer implements Listener {
	...
	private static final long REFRESH_DELAY_TICKS = 2L; // time in ticks it will update the remaining time on the
														// Enderpearl.
	private static final long REFRESH_DELAY_TICKS_18 = 20L; // time in ticks it will update the remaining time on the
															// Enderpearl for a 1.8 client.
	private static final long EXPIRE_SHOW_MILLISECONDS = 1500L; // time in milliseconds it will show the Enderpearl
																// cooldown is over on the hotbar for

	// private final Map<UUID, PearlNameFaker> itemNameFakes = new HashMap<>();
	private final JavaPlugin plugin;

	public EnderPearlTimer(JavaPlugin plugin) {
		super(HCF.getPlugin().getMessagesYML().getString("SCOREBOARD.ENDERPEARL.NAME"),
				TimeUnit.SECONDS.toMillis(HCF.getPlugin().getMessagesYML().getLong("SCOREBOARD.ENDERPEARL.LENGTH")));
		this.plugin = plugin;
	}
	...
	public void onDisable(Config config) {
		...
		// for (Iterator<PearlNameFaker> iterator = itemNameFakes.values().iterator();
		// iterator.hasNext();) {
	...
	public void onProjectileLaunch(ProjectileLaunchEvent event) {
		...
		if (projectile instanceof EnderPearl) {
			...
			if (source instanceof Player) {
				...
				if (remaining > 0L) {
					shooter.sendMessage(Utils.chat(ChatColor.RED + "You may " + getDisplayName() + ChatColor.RED
							+ " in another " + ChatColor.BOLD + DurationFormatter.getRemaining(remaining, true, false)
							+ ChatColor.RED + '.'));
					if (!plugin.getConfig().getBoolean("modified-launch-event")) {
						shooter.getInventory().getItemInHand()
								.setAmount(shooter.getInventory().getItemInHand().getAmount() + 1);
					}
	...
	// pearlNameFaker.setFakeItem(CraftItemStack.asNMSCopy(event.getCurrentItem()),
	// event.getSlot());
	// new BukkitRunnable() {
	// @Override
	// public void run() {
	...
	// itemNameFakes.putIfAbsent(player.getUniqueId(), pearlNameFaker = new
	// PearlNameFaker(this, player)) == null) {
	// long ticks = ((CraftPlayer)
	// player).getHandle().playerConnection.networkManager.getVersion() >= 47 ?
	// REFRESH_DELAY_TICKS_18 : REFRESH_DELAY_TICKS;
	...
	// net.minecraft.server.v1_7_R4.ItemStack item = ((CraftItemStack)
	// stack).handle;
	// if (remaining > 0L) {
	// item = item.cloneItemStack();
	// item.c(ChatColor.LIGHT_PURPLE + "Enderpearl Cooldown: " + ChatColor.RED +
	...
	// super.cancel();
	// setFakeItem(CraftItemStack.asNMSCopy(player.getItemInHand()),
	// player.getInventory().getHeldItemSlot()); // show the original item here.
	// }
	 ...
	 */
	// public void setFakeItem(net.minecraft.server.v1_7_R4.ItemStack nms, int
	// index) {
	...
	// entityPlayer.playerConnection.sendPacket(new
	// PacketPlayOutSetSlot(entityPlayer.activeContainer.windowId, index, nms));
	// }
	// }
}
