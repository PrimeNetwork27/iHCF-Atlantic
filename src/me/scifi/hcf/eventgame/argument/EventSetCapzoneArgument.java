...
/**
 * An {@link CommandArgument} used for setting the {@link CaptureZone}s of an
 * {@link EventFaction}.
 */
public class EventSetCapzoneArgument extends CommandArgument {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if (selection.getWidth() < CaptureZone.MINIMUM_SIZE_AREA
				|| selection.getLength() < CaptureZone.MINIMUM_SIZE_AREA) {
			sender.sendMessage(ChatColor.RED + "Capture zones must be at least " + CaptureZone.MINIMUM_SIZE_AREA + 'x'
					+ CaptureZone.MINIMUM_SIZE_AREA + '.');
			return true;
		}
		...
		if (capturableFaction instanceof ConquestFaction) {
			if (args.length < 3) {
				sender.sendMessage(ChatColor.RED + "Usage: /" + label + ' ' + getName() + ' ' + faction.getName()
