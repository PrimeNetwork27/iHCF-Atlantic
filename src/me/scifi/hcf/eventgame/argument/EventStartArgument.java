...
public class EventStartArgument extends CommandArgument {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if (plugin.getManagerHandler().getTimerManager().getEventTimer().tryContesting(((EventFaction) faction),
				sender)) {
			sender.sendMessage(ChatColor.YELLOW + "Successfully contested " + faction.getName() + '.');
			GameStartEvent event = new GameStartEvent((EventFaction) faction);
			event.call();
		}
	...
	public List<String> onTabComplete(CommandSender sender, Command command, String label, String[] args) {
		...
		return plugin.getManagerHandler().getFactionManager().getFactions().stream()
				.filter(faction -> faction instanceof EventFaction).map(Faction::getName).collect(Collectors.toList());
	}
}
