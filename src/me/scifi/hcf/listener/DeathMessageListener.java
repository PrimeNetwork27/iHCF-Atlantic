...
public class DeathMessageListener implements Listener {
	...
	private String getDeathMessage(String input, Entity entity, Entity killer) {
		...
		if (entity != null) {
			input = input.replaceFirst("(?i)" + getEntityName(entity),
					ChatColor.RED + getDisplayName(entity) + ChatColor.YELLOW);
		}

		if (killer != null && (entity == null || !killer.equals(entity))) {
			input = input.replaceFirst("(?i)" + getEntityName(killer),
					ChatColor.RED + getDisplayName(killer) + ChatColor.YELLOW);
		}
	...
	private String getDisplayName(Entity entity) {
		...
		if (entity instanceof Player) {
			...
			return player.getName() + ChatColor.GOLD + '[' + ChatColor.WHITE
					+ plugin.getManagerHandler().getUserManager().getUser(player.getUniqueId()).getKills()
					+ ChatColor.GOLD + ']';
		} else {
