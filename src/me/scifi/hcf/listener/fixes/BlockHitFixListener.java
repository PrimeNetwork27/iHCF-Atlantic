...
/**
 * Prevents players from breaking a break and quickly attacking a player through
 * the block before it regenerates.
 */
public class BlockHitFixListener implements Listener {
	...
	private static final ImmutableSet<Material> NON_TRANSPARENT_ATTACK_BREAK_TYPES = Sets
			.immutableEnumSet(Material.GLASS, Material.STAINED_GLASS, Material.STAINED_GLASS_PANE);
	...
	public void onEntityDamageByEntity(EntityDamageEvent event) {
		...
		if (attacker != null) {
			...
			if (lastInteractTime != this.lastInteractTimes.getNoEntryValue()
					&& lastInteractTime - System.currentTimeMillis() > 0L) {
