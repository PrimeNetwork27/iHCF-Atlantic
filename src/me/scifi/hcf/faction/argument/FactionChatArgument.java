...
public class FactionChatArgument extends CommandArgument {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if (parsed == null && currentChannel != ChatChannel.PUBLIC) {
			...
			String format = String.format(currentChannel.getRawFormat(player), "",
		...
		sender.sendMessage(plugin.getMessagesYML().getString("FACTION-CHAT-CHANGED").replace("{chatChannel}",
				newChannel.getDisplayName().toLowerCase()));
		return true;
	}
