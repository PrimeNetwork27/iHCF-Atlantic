...
public class FactionBoxArgument extends CommandArgument {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if (plugin.getManagerHandler().getFactionManager().getPlayerFaction(p) == null) {
			p.sendMessage(plugin.getMessagesYML().getString("FACTION-PLAYER-COMMAND-FACTION"));
			return true;
		}

		if (plugin.getManagerHandler().getFactionManager().getPlayerFaction(p).getClaims().isEmpty()) {
			p.sendMessage(plugin.getMessagesYML().getString("FACTION-PLAYER-COMMAND-CLAIM"));
			return true;
		}

		if (redeemed.contains(plugin.getManagerHandler().getFactionManager().getPlayerFaction(p).getUniqueID())) {
			p.sendMessage(plugin.getMessagesYML().getString("FACTION-PLAYER-COMMAND-ALREADY"));
			return true;
		}
		plugin.getManagerHandler().getFactionManager().getPlayerFaction(p).getClaims().forEach(claim -> {
			int y = 80;
			while (y > 0) {
				Cuboid cuboid = new Cuboid(new Location(claim.getWorld(), claim.getX1(), claim.getY1(), claim.getZ1()),
						new Location(claim.getWorld(), claim.getX2(), claim.getY2(), claim.getZ2()));
				// Side One
