...
public class FactionWithdrawArgument extends CommandArgument {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if (factionMember.getRole() == Role.MEMBER) {
			sender.sendMessage(plugin.getMessagesYML().getString("FACTION-WITHDRAW-MUST"));
			return true;
		}
		...
		if (args[1].equalsIgnoreCase("all")) {
			...
			if ((amount = (JavaUtils.tryParseInt(args[1]))) == null) {
				sender.sendMessage(plugin.getMessagesYML().getString("FACTION-WITHDRAW-VALID").replace("{1}", args[1]));
				return true;
			}
		}
		...
		if (amount > factionBalance) {
			sender.sendMessage(plugin.getMessagesYML().getString("FACTION-WITHDRAW-LEAST")
					.replace("{amount}", EconomyManager.ECONOMY_SYMBOL + JavaUtils.format(amount))
					.replace("{balance}", EconomyManager.ECONOMY_SYMBOL + JavaUtils.format(factionBalance)));
		...
		playerFaction.broadcast(plugin.getMessagesYML().getString("FACTION-WITHDRAW-BROADCAST")
				.replace("{sender}",
