...
public enum ChatChannel {
	 ...
	 * @return the parsed {@link ChatChannel} or {@link ChatChannel#PUBLIC} if not
	 *         found
	 */
	...
	public String getRawFormat(final Player player) {
		...
		case FACTION: {
			return ChatColor.translateAlternateColorCodes('&',
					HCF.getPlugin().getMessagesYML().getString("FACTION-CHAT-FACTION")
							.replace("{player}", player.getName()).replace("{message}", "%2$s"));
		}
		case ALLIANCE: {
			return ChatColor.translateAlternateColorCodes('&', HCF.getPlugin().getMessagesYML()
					.getString("FACTION-CHAT-ALLY").replace("{player}", player.getName()).replace("{message}", "%2$s"));
		}
		default: {
