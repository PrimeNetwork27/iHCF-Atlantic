...
public class DeathbanListener implements Listener {
	...
	public void onPlayerLogin(PlayerLoginEvent event) {
		...
		if (!plugin.getConfig().getBoolean("mongo.enabled")) {
			...
			// If the user has tried joining in the last 30 seconds and kicked for deathban
			// but has lives, let them join this time to prevent accidental life use.
			if (lastAttemptedJoinMillis != this.lastAttemptedJoinMap.getNoEntryValue()
					&& lastAttemptedJoinMillis - millis < DeathbanListener.LIFE_USE_DELAY_MILLIS) {
				this.lastAttemptedJoinMap.remove(uuid);
				user.removeDeathban();
				lives = plugin.getManagerHandler().getDeathbanManager().takeLives(uuid, 1);

				event.setResult(PlayerLoginEvent.Result.ALLOWED);
				new DelayedMessageRunnable(player, ChatColor.YELLOW + "You have used a life for entry. You now have "
						+ ChatColor.WHITE + lives + ChatColor.YELLOW + " lives.").runTask(plugin);
	...
	public void onPlayerDeath(PlayerDeathEvent event) {
		...
		Deathban deathban = plugin.getManagerHandler().getDeathbanManager().applyDeathBan(player,
	...
	public void onPlayerRequestRespawn(PlayerRespawnEvent event) {
		...
		if (deathban != null && deathban.getRemaining() > 0L) {
			if (player.hasPermission(DeathbanListener.DEATH_BAN_BYPASS_PERMISSION)) {
				...
				user.removeDeathban();
				new DelayedMessageRunnable(player, ChatColor.RED + "You would be death-banned for "
						+ deathban.getReason() + ChatColor.RED + ", but you have access to bypass.")
						.runTask(this.plugin);
	...
	private void handleKick(Player player, Deathban deathban) {
		if (this.plugin.getManagerHandler().getEotwHandler().isEndOfTheWorld()) {
			...
			player.kickPlayer(ChatColor.RED + "Deathbanned for "
					+ DurationFormatter.getRemaining(deathban.getRemaining(), true, false) + ": " + ChatColor.WHITE
					+ deathban.getReason());
		}
	}
}
