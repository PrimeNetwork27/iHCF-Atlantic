...
public class VisualiseHandler implements IManager {
	...
	/**
	 * Gets the current {@link VisualBlock}s to their {@link Location}s that are
	 * shown to a {@link Player} of a specific {@link VisualType}.
	 *
	 * @param player the {@link Player} to get for
	 ...
	 *                          (this is used to prevent unnecessary packets sent
	 *                          when disconnecting or changing worlds, for example)
	 * @return if the visual block was shown in the first place
	 */
	public boolean clearVisualBlock(Player player, Location location, boolean sendRemovalPacket) {
		synchronized (storedVisualises) {
			...
			if (sendRemovalPacket && visualBlock != null) {
				// Have to send a packet to the original block type, don't send if the fake
				// block has the same data properties though.
	...
	public Map<Location, VisualBlock> clearVisualBlocks(Player player, @Nullable VisualType visualType,
		...
		synchronized (storedVisualises) {
			...
			Map<Location, VisualBlock> results = new HashMap<>(this.storedVisualises.row(player.getUniqueId())); // copy
																													// to
																													// prevent
																													// commodification
			...
			for (Map.Entry<Location, VisualBlock> entry : results.entrySet()) {
				...
				if ((predicate == null || predicate.apply(visualBlock))
					...
					if (removed.put(location, visualBlock) == null) { // not really necessary, but might as well
						this.clearVisualBlock(player, location, sendRemovalPackets); // this will call remove on
																						// storedVisualises.
					}
				}
			}
